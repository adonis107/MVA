...
\begin{frame}{Geometric Approach: $\epsilon$-Thickened Rectangles}
    How do we find these "faces" with finite data? We introduce a tolerance $\epsilon$.
    ...
    \begin{columns}
        ...
        \column{0.5\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/structure_extreme_data.png} 
        \small{Visualizing mass concentration on low-dim cones}
    \end{columns}

    \vspace{0.5cm}
    \textbf{The Estimator:} We count how many extreme points fall into each cone. $$ \hat{\mathcal{M}}(\alpha) = \frac{1}{k} \sum_{i=1}^n \mathbb{I} \left\{ \text{Point } i \in \text{Cone } \alpha \right\} $$
\end{frame}
