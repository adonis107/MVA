...
\author{%
  Adonis Jamal \\
  ENS Paris-Saclay \\
  \texttt{\small adonis.jamal@ens-paris-saclay.fr}\\
  \And
...
\begin{abstract}
  This paper analyzes Hoffman and Gelman's "The No-U-Turn Sampler" \cite{main}, an extension of Hamiltonian Monte Carlo that eliminates manual path-length tuning by adaptively setting it during sampling. We critically evaluate the strengths, weaknesses, and practical limitations of NUTS using our own implementation. Code is available at \url{https://github.com/adonis107/MVA-BML}.
\end{abstract}
...
\subsection{Theoretical Complements}

\subsubsection{Rigorous Proof of Uniform Distribution over the Candidate Set}

In the original paper, the uniformity of the conditional distribution over the candidate set $\mathcal{C}$ (Equation (2) in the paper) is derived very concisely from conditions C.1 to C.4. The argument is compressed into a few proportionality statements. We restate it more explicitly.
...
\]
as required. \todo{pareil a voir si on garde}

\subsubsection{The Transition Kernel and Proof of Detailed Balance}

We now turn to the second key ingredient ensuring correctness of Efficient NUTS, the memory-efficient transition kernel, whose role is to preserve the uniform distribution over the candidate set $\mathcal{C}$ while enabling incremental sampling from newly constructed subtrees.
...
\subsection{Strengths and Weaknesses of the Article}

As explained, the major strength of the article is that it resolves a central practical limitation of HMC, the manual tuning of the trajectory length through a conceptually simple yet theoretically rigorous construction. The No-U-Turn criterion and recursive doubling scheme are carefully designed to preserve detailed balance, and the integration of slice sampling ensures exactness despite adaptive stopping. The addition of dual averaging for step-size adaptation makes the method essentially tuning-free, greatly improving usability. 

However, some limitations remain. The dual-averaging scheme introduces hyperparameters whose default values are justified empirically rather than theoretically, and performance guarantees are mainly asymptotic, with limited insight into convergence rates. The tree-building procedure adds computational overhead and may waste gradient evaluations when trajectories are long. Finally, although the experiments are extensive, they cover only a limited range of models, leaving the behavior of NUTS on more pathological or highly multimodal targets less explored as we will see here.

\subsection{Practical Limitations}\todo{a voir si on garde les sous titres ca prends de la place}
